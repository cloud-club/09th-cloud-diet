...
> **회사**: CloudSync · **인수자**: @weeeeestern
> Week 2 · 데드라인: 2026-05-18 (월요일 22:00 세션 전까지)

## 상황

dev/staging/prod RDS 인스턴스 12개. dev에 Multi-AZ가 켜져 있고, backup retention이 35일이고, Read Replica를 만들었는데 미사용이고, Lambda → RDS 연결에 RDS Proxy가 없습니다. RDS Proxy 부재 영향은 Lambda concurrency 패턴을 같이 봐야 결론이 나옵니다.

이번 주차는 **단일 에이전트로는 풀기 어려운 결합 시나리오**를 받았습니다. 분석할 대상이 `main.tf` 한 파일에 모두 들어 있는데, 라인 수가 약 **453줄**이고 여러 도메인이 섞여 있습니다.
...
| 파일 | 내용 |
|------|------|
| `main.tf` | 결합된 Terraform (약 453줄, 컴포넌트별 prefix `comp1_` ~ `comp4_`) |
| `cost_report.json` | 6개월 비용 히스토리 + 서비스별 breakdown · 월 낭비 추정 약 **$2,173** |
...
`Submit Answer` 페이지에서 시나리오 ID `MA-006`로 다음을 제출:
...
DB + Backup/DR + Application 3 전문가. Application 전문가가 connection pool exhaustion을 발견해야 DB 전문가가 'RDS Proxy 도입 시 다운사이즈 가능' 결론에 도달합니다.
