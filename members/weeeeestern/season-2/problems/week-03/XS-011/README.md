# XS-011 · RDS Read Replica 잘못된 region (transfer 폭증)

> **회사**: AppForge · **인수자**: @weeeeestern
> Week 2 · 데드라인: 2026-05-18 (월요일 22:00 세션 전까지)

## 상황

DR 목적으로 cross-region replica 구축. 그런데 dev팀이 read traffic을 misconfiguration으로 cross-region replica로 보내도록 설정. 게다가 같은 dev RDS는 Multi-AZ까지 켜져있음. replica 비용 + cross-region transfer 이중 함정.

이번 주차는 **단일 에이전트로는 풀기 어려운 결합 시나리오**를 받았습니다. 분석할 대상이 `main.tf` 한 파일에 모두 들어 있는데, 라인 수가 약 **174줄**이고 여러 도메인이 섞여 있습니다.
...
| 파일 | 내용 |
|------|------|
| `main.tf` | 결합된 Terraform (약 174줄, 컴포넌트별 prefix `comp1_` ~ `comp2_`) |
| `cost_report.json` | 6개월 비용 히스토리 + 서비스별 breakdown · 월 낭비 추정 약 **$1,403** |
...
`Submit Answer` 페이지에서 시나리오 ID `XS-011`로 다음을 제출:
...
DB 전문가가 replica endpoint 사용 패턴을, Network 전문가가 VPC Flow Logs cross-region direction을 함께 보면 misconfiguration 식별 가능.
